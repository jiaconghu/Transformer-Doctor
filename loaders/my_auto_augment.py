...
from loaders import image_ops, image_enhance
import PIL
import numpy as np
...
def shear_x(imgs, factor, **kwargs):
    _check_args_tf(kwargs)
    trans = []
    for img in imgs:
        trans.append(img.transform(img.size, Image.AFFINE, (1, factor, 0, 0, 1, 0), **kwargs))
    return trans


def shear_y(imgs, factor, **kwargs):
    _check_args_tf(kwargs)
    trans = []
    for img in imgs:
        trans.append(img.transform(img.size, Image.AFFINE, (1, 0, 0, factor, 1, 0), **kwargs))
    return trans


def translate_x_rel(imgs, pct, **kwargs):
    _check_args_tf(kwargs)
    trans = []
    for img in imgs:
        pixels = pct * img.size[0]
        trans.append(img.transform(img.size, Image.AFFINE, (1, 0, pixels, 0, 1, 0), **kwargs))
    return trans


def translate_y_rel(imgs, pct, **kwargs):
    _check_args_tf(kwargs)
    trans = []
    for img in imgs:
        pixels = pct * img.size[1]
        trans.append(img.transform(img.size, Image.AFFINE, (1, 0, 0, 0, 1, pixels), **kwargs))
    return trans
...
def rotate(imgs, degrees, **kwargs):
    _check_args_tf(kwargs)
    if _PIL_VER >= (5, 2):
        trans = []
        for img in imgs:
            trans.append(img.rotate(degrees, **kwargs))
        return trans
    if _PIL_VER >= (5, 0):
        w, h = img.size
        post_trans = (0, 0)
...
def auto_contrast(imgs, **__):
    return image_ops.autocontrast(imgs)


def invert(imgs, **__):
    return image_ops.invert(imgs)


def equalize(imgs, **__):
    return image_ops.equalize(imgs)


def solarize(imgs, thresh, **__):
    return image_ops.solarize(imgs, thresh)


def solarize_add(imgs, add, thresh=128, **__):
    img = imgs[0]
    lut = []
    for i in range(256):
        if i < thresh:
...
def posterize(imgs, bits_to_keep, **__):
    if bits_to_keep >= 8:
        return imgs
    return image_ops.posterize(imgs, bits_to_keep)


def contrast(imgs, factor, **__):
    return image_enhance.MyContrast(imgs).enhance(factor)


def color(imgs, factor, **__):
    return image_enhance.MyColor(imgs).enhance(factor)


def brightness(imgs, factor, **__):
    return image_enhance.MyBrightness(imgs).enhance(factor)


def sharpness(imgs, factor, **__):
    return image_enhance.MySharpness(imgs).enhance(factor)
