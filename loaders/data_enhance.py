...
from loaders.image_masks_transforms import Normalize, ToTensor, Resize, RandomHorizontalFlip, Compose, ColorJitter,\
     MyRandomResizedCropAndInterpolation, MyRandomErasing
from loaders.my_auto_augment import my_rand_augment_transform
...
def transforms_imagenet_train_mask(
        img_size=224,
        scale=None,
        ratio=None,
        hflip=0.5,
    ...
    ratio = tuple(ratio or (3./4., 4./3.))  # default imagenet ratio range
    primary_tfl = [
        MyRandomResizedCropAndInterpolation(img_size, scale=scale, ratio=ratio, interpolation=interpolation)]
    if hflip > 0.:
        primary_tfl += [RandomHorizontalFlip(p=hflip)]
    if vflip > 0.:
        primary_tfl += [RandomVerticalFlip(p=vflip)]
    ...
    if auto_augment:
        ...
        if interpolation and interpolation != 'random':
            aa_params['interpolation'] = str_to_pil_interp(interpolation)
        if auto_augment.startswith('rand'):
            secondary_tfl += [my_rand_augment_transform(auto_augment, aa_params)]
        elif auto_augment.startswith('augmix'):
            aa_params['translate_pct'] = 0.3
            secondary_tfl += [augment_and_mix_transform(auto_augment, aa_params)]
        else:
    ...
    if use_prefetcher:
        ...
        if re_prob > 0.:
            final_tfl.append(
                MyRandomErasing(re_prob, mode=re_mode, max_count=re_count, num_splits=re_num_splits, device='cpu'))
...
def create_transform_mask(
    ...
    if tf_preprocessing and use_prefetcher:
        ...
        if is_training and no_aug:
            ...
            transform = transforms_imagenet_train_mask(
                img_size,
                scale=scale,
                ratio=ratio,
                hflip=hflip,
